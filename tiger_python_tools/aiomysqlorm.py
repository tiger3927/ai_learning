...
async def create_pool(loop, **kw):
    ...
    __pool = await aiomysql.create_pool(
        ...
        maxsize=kw.get('maxsize', 32),  # 设置最大连接数，默认10
...
if __name__ == "__main__":
    ...
    class region(Model):
        region_id = IntegerField('region_id', primary_key=True)
    ...
    @asyncio.coroutine
    def test():
        yield from create_pool(loop=loop, host='a10.4s188.com', port=35106, user='root', password='tongji12', db='jt808')
        #nr = region(region_id=8, region_name='sly')
        #yield from user.save()
        #r = yield from region.find('11')
        #print(r)
        #r = yield from region.findAll()
        #print(1, r)
        #k=yield from region.findNumber(region_id='12')
        #print(k)
        r = yield from region.findAll(region_id='12')
        for a in r:
            print(a)
